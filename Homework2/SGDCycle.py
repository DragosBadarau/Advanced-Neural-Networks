...
train_transforms = v2.Compose([
    v2.ToImage(),
    v2.ToDtype(torch.float32, scale=True),
    v2.RandomRotation(degrees=10),  # Mild rotation to add more variety
    v2.Normalize((0.5, 0.5, 0.5), (0.25, 0.25, 0.25), inplace=True)
])
...
optimizer = torch.optim.SGD(model.parameters(), lr=0.0075, momentum=0.5, weight_decay=0.0025)
def train():
    model.train()
    correct = 0
    total = 0
...
best = 0.0
patience = 11  # Number of epochs to wait for improvement
epochs_no_improve = 0
epochs = list(range(50))
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer,
                                                max_lr=0.0075,  # Maximum learning rate
                                                steps_per_epoch=len(train_loader),
                                                epochs=30,  # Use fewer epochs if overfitting is detected early
                                                pct_start=0.3,  # Learning rate ramps up for the first 30% of training
                                                anneal_strategy='cos',  # Cosine annealing for smooth reduction
